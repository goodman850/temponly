...
logging.config.fileConfig('/var/www/html/p/log/loggerConfig.ini')
...
def main():
    ...
    try:
        ...
        file = open("/var/www/html/p/log/das","r")
        text = file.read()
        file.close()
        if(len(text)>3 and text.find("}}") != -1):
            newfile = open("/var/www/html/p/log/das","w")
